...
class RemoveCertificateAction(BaseAction):
    """Remove mitmproxy CA certificate from the Windows trust store."""
    ...
    def execute(self, command: Command, context: dict) -> CommandResult:
        ...
        if ok:
            self.handle_success(command, result,
                                f"[remove_certificate] {msg} | Result: PASSED")
        else:
            self.handle_failure(command, result,
                                f"[remove_certificate] {msg} | Result: FAILED")
        return result
